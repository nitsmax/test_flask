...
class EmbeddingIntentClassifier():
    ...
    def prepare_training_data(self,X,y):
        ...
        print('training_data: ', training_data, type(training_data))
    ...
    def train(self, X,y):
        ...
        print('inv_intent_dict: ', self.inv_intent_dict)
        

        self.encoded_all_intents = self._create_encoded_intents(
                                        intent_dict)

        print('encoded_all_intents: ', self.encoded_all_intents, self.encoded_all_intents.shape)
        
        X, Y, helper_data = self._prepare_data_for_training(
                                training_data, intent_dict)
        print('_prepare_data_for_training: ')
        print('X :', X)
        print('y: ', Y)
        print('helper_data: ', helper_data)
        print('X.shape :',X.shape, X.shape[-1])
        print('Y.shape: ', Y.shape, Y.shape[-1])
        return
